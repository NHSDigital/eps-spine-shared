...
def check_prescriber_details(context, log_object: EpsLogger, internal_id):
    ...
    if not constants.REGEX_ALPHANUMERIC8.match(context.msg_output[message_vocab.AGENT_PERSON]):
        ...
        if not constants.REGEX_ALPHANUMERIC12.match(context.msg_output[message_vocab.AGENT_PERSON]):
...
def check_repeat_dispense_instances(context, log_object: EpsLogger, internal_id):
    ...
    context.msg_output[message_vocab.REPEATLOW] = int(context.msg_output[message_vocab.REPEATLOW])
    context.msg_output[message_vocab.REPEATHIGH] = int(context.msg_output[message_vocab.REPEATHIGH])
    ...
    if context.msg_output[message_vocab.REPEATHIGH] > constants.MAX_PRESCRIPTIONREPEATS:
        ...
        raise EpsValidationError(supp_info)
    if context.msg_output[message_vocab.REPEATHIGH] < context.msg_output[message_vocab.REPEATLOW]:
